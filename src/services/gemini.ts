...
try {
  ...
  if (fs.existsSync(policyPath)) {
    ...
    logger.info('[Gemini] Company policy loaded successfully from policy.md');
  } else {
    logger.warn('[Gemini] Policy file not found at:', policyPath);
  }
} catch (error) {
  logger.error('[Gemini] Error loading company policy:', error);
}
