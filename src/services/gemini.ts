...
9. **สำคัญ:** หากผู้ใช้สอบถามหรือแจ้งปัญหาเกี่ยวกับ **เลขเอกสารจากทุกระบบ** (เช่น Salesforce ขึ้นต้นด้วย CR, IV-, M- หรือระบบ SAP หรืออื่นๆ) รวมทั้งหากต้องการขอ **แก้ไขเอกสารทุกชนิดที่ Approved หรือทำรายการไปแล้ว** ให้ตอบผู้ใช้อย่างสุภาพว่า "เนื่องจากการแก้ไขเอกสารในระบบจำเป็นต้องมีการเก็บหลักฐานเพื่อใช้ในการ Audit รบกวนส่งอีเมลรายละเอียดแจ้งเรื่องไปที่ it@jastel.co.th เพื่อให้ทีมงานดำเนินการตรวจสอบและบันทึกข้อมูลเข้าระบบให้นะครับ"
...
- ถ้าปัญหากว้างเกินไป ให้แนะนำการเช็กพื้นฐานกลางๆ`;
  ...
  async categorizeIssue(issueSummary: string, context?: RequestContext): Promise<{ category: string; subCategory: string }> {
    ...
    await this.analysisSemaphore.acquire();
    try {
      const prompt = `จากปัญหา IT ต่อไปนี้: "${this.sanitizeText(issueSummary)}"
กรุณาจัดหมวดหมู่ให้เป็นระเบียบตามรูปแบบ JSON ดังนี้:
{
  "category": "หมวดหมู่หลัก (ภาษาอังกฤษ)",
  "subCategory": "หมวดหมู่ย่อย (ภาษาอังกฤษ)"
}

หมวดหมู่หลักที่แนะนำ (ถ้ามีที่ใกล้เคียงกว่าให้ใช้ตามความเหมาะสม):
