...
export class GeminiService {
  ...
  private sanitizeReferenceText(text: string): string {
    return this.sanitizeText(text).replace(/[<>]/g, '');
  }
  ...
  private async retryWithBackoff<T>(
    ...
    try {
      ...
      if (retries > 0 && this.isRetryableError(error)) {
        ...
        logger.warn(
          `Gemini retry in ${Math.round(totalDelay)}ms (${retries} retries left) | Load: ${this.chatSemaphore.currentLoad}/${this.chatSemaphore.queueLength} queued`
        );
  ...
  async analyzeImage(
    ...
    try {
      return await this._analyzeImageInternal(base64Image, userText, mimeType);
    } finally {
      this.analysisSemaphore.release();
    }
  }

  private async _analyzeImageInternal(
    base64Image: string,
    userText?: string,
    mimeType: string = 'image/jpeg',
  ): Promise<GeminiAnalysisResult> {
