...
7. ห้ามแนะนำ Software ผิดลิขสิทธิ์ ห้ามเปิดเผยข้อมูลส่วนตัว
  ...
  private async _chatInternal(conversationHistory: IMessage[]): Promise<string> {
    return this.retryWithBackoff(async () => {
      try {
        ...
        // ถ้าเป็น Rate Limit (429) ให้แจ้งว่าระบบยุ่ง
        if (err?.status === 429 || err?.message?.includes('429')) {
          return 'ขออภัยครับ ขณะนี้มีผู้ใช้งานจำนวนมาก กรุณารอสักครู่แล้วลองถามใหม่อีกครั้งครับ [[TYPE:IT_PROBLEM]] [[TOPIC:ไม่ระบุ]]';
        }
